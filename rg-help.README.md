...
Help output captured from `rg --help` for the ripgrep build bundled with
Cursor (`ripgrep 15.1.0-cursor5`, rev `c3e3c2f7ec`).

This dump includes:

- The Cursor-specific `--cursor-ignore=PATH` flag, which is layered on top of
  the usual gitignore / .ignore / .rgignore precedence rules
- The extended `--hyperlink-format=FORMAT` flag with the `cursor` alias
  listed alongside the standard `default`, `file`, `vscode`, `vscodium`,
  `kitty`, `macvim`, `textmate`, and `grep+` aliases
- The full flag reference for the upstream ripgrep 15.1 CLI surface

## Raw content

```text
ripgrep 15.1.0-cursor5 (rev c3e3c2f7ec)
...
FILTER OPTIONS:
    ...
    --cursor-ignore=PATH
        Specifies a path to one or more gitignore-formatted rules files that
        are applied with higher precedence than -g/--glob overrides. These
        rules can exclude files even when a glob would otherwise include them.
        Multiple files may be specified by repeating this flag; later files
        have higher precedence.

        Whitelist rules (lines starting with !) override ignore rules from
        every other ignore source, including .gitignore, .ignore, .rgignore,
        --ignore-file, global gitignore, and .git/info/exclude.

        This behaves like --ignore-file except for its higher precedence.
...
OUTPUT OPTIONS:
    ...
    --sort=SORTBY
        ...
        path: Sort by file path. The order is determined by sorting files in
        each directory entry during traversal. This means that given the files
        a/b and a+, the latter will sort after the former even though + would
        normally sort before /.
        ...
        When sorting is requested, ripgrep may buffer results in memory and
        print them at the end in the requested order. This can increase memory
        usage and reduce streaming behavior, but searching itself may still run
        in parallel.
    ...
    --sortr=SORTBY
        ...
        path: Sort by file path. The order is determined by sorting files in
        each directory entry during traversal. This means that given the files
        a/b and a+, the latter will sort before the former even though + would
        normally sort after / when doing a reverse lexicographic sort.
    ...
    --sort-files
        ...
        lexicographically in ascending order. When sorting is requested,
        ripgrep may buffer results in memory and print them at the end.
        Searching itself may still run in parallel.
